...
import { DESTROY_SPRINT } from '../../../../services/Mutations/SprintMutation';

export const ListSprint = (props) => {
    const [showModal, setShowModal] = useState(false);

    const [deleteSprint] = useMutation(DESTROY_SPRINT, {
        onCompleted(){
            ...
            <tr>
                ...
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <p className='text-gray-900 whitespace-no-wrap'>
                        {
                            props.description.split('').length <= 20 ? props.description : props.description.slice(0, 20) + ' ...'
                        }
                    </p>
                </td>
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                ...
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <p className='text-gray-900 whitespace-no-wrap'>{props.team}</p>
                </td>
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <p className='text-gray-900 whitespace-no-wrap'>{props.status}</p>
                </td>
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <p className='text-gray-900 whitespace-no-wrap'>{props.project}</p>
                </td>
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <p className='text-gray-900 whitespace-no-wrap'>{props.environment}</p>
                </td>
